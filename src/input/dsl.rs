...
#[inline]
pub fn sleep_ms(ms: u64) -> InputStep {
    InputStep::Sleep(Duration::from_millis(ms))
}

#[inline]
pub fn sleep(d: Duration) -> InputStep {
    InputStep::Sleep(d)
}
...
pub fn tap(k: Key) -> Vec<InputStep> {
    let mut v = Vec::new();
    if let Some(s) = k.to_step_down() {
        v.push(s);
    }
    if let Some(s) = k.to_step_up() {
        v.push(s);
    }
    v
}
...
#[inline]
pub fn down(k: Key) -> Option<InputStep> {
    k.to_step_down()
}
#[inline]
pub fn up(k: Key) -> Option<InputStep> {
    k.to_step_up()
}
...
pub fn hold(mods: &[Key], main: Key, ms: u64) -> Vec<InputStep> {
    ...
    if let Some(s) = down(main) {
        v.push(s)
    }
    v.push(sleep_ms(ms));
    if let Some(s) = up(main) {
        v.push(s)
    }
    for &m in mods.iter().rev() {
        if let Some(s) = up(m) {
...
pub fn click_n(btn: MouseButton, n: usize, between: Option<Duration>) -> Vec<InputStep> {
    ...
    for i in 0..n {
        ...
        if i + 1 != n {
            if let Some(d) = between {
                v.push(InputStep::Sleep(d));
            }
        }
    }
    v
